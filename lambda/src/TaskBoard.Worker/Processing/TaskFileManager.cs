...
public sealed partial class TaskFileManager(ILogger<TaskFileManager> logger)
{
    ...
    /// <summary>
    /// Classifies a comment as agent-generated. Accepts both the new unified
    /// <c>aiboard-log</c> marker (rerun redesign Problem 2) and the legacy
    /// per-shape markers (<c>agent-step:</c>, <c>agent-run:</c>, etc.) so the
    /// migration can land in stages — chronological-log call sites move to
    /// <c>aiboard-log</c> first; status-notice sites (rate-limit, shutdown,
    /// dependency-blocked, etc.) follow as the codebase converges. Both
    /// shapes classify identically: agent-generated, excluded from
    /// operator-comment hash inputs.
    /// </summary>
    internal static bool ContainsAgentMarker(string body)
        => AiboardLogMarker.IsAgentGenerated(body)
