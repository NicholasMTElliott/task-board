...
- ✅ Pre-flight config validation (`StartupConfigValidator`): runs after config merge, before tenant resolution, via temporary pre-host logger. **Errors** (exit): unknown `BoardProvider`; `BoardProvider=github` missing any of Owner/Repo/ProjectNumber (all listed at once); `BoardProvider=trello` missing any of ApiKey/ApiToken/BoardId. **Warnings** (continue): `GitHubProjects` populated but provider≠github; `Trello` populated but provider≠trello; both sections populated (names the winner); unrecognized `AgentExecutor` value. Chose NOT to flag missing `WorktreeBasePath`/`PollIntervalSeconds`/`ClaudeCli:ExecutablePath`/stub `Stub:TenantName` (all have sensible defaults). `TenantIdentifierFactory.Create` widened to list all missing keys in one exception instead of first-missing.
