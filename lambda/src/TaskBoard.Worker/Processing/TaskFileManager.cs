...
public sealed partial class TaskFileManager(ILogger<TaskFileManager> logger)
{
    ...
    /// <summary>
    /// Classifies a comment as agent-generated by checking for the unified
    /// <c>aiboard-log</c> marker (rerun redesign Problem 2). Legacy per-shape
    /// markers (<c>agent-step:</c>, <c>agent-run:</c>, etc.) are no longer
    /// recognised — the new system emits only <c>aiboard-log</c>; existing
    /// development cards are recreated rather than migrated.
    /// </summary>
