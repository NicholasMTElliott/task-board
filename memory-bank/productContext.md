...
- Natural task sequencing is manual → dependencies block pickup until prerequisite tickets are satisfied
...
6. *(Stories only)* Tasking decomposes the story into child tasks. Each child inherits the story's priority, gets a best-guess estimate, and is placed in **Ready for Design**. The story's estimate is set to the sum of child estimates. Story moves to **Waiting for Tasks** and completes automatically when all children reach **Done**.
   - If generated tasks have hard sequencing constraints, tasking writes `blockedBy` / `blocks` front matter so downstream polling skips blocked tasks.
...
- Blocked tickets remain visible in their Ready/Approved column with a dependency comment; the runner selects other available work.
