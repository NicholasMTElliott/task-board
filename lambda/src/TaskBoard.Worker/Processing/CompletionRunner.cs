        ...
        string cardId, string boardId, string workspacePath, CancellationToken ct)
    {
        ...
        if (childRefs.Count == 0)
        {
            ...
            if (state.Transitions.TryGetValue(TransitionKeys.Error, out var errorTarget))
            {
                ...
                await TransitionExecutor.ExecuteAsync(cardId, errorTarget, boardClient, logger, ct, templateContext, crossReferenceResolver);
            }
        ...
        if (pending.Count == 0)
        {
            ...
            {
                var templateContext = await BuildTemplateContextAsync(ct);
                await TransitionExecutor.ExecuteAsync(cardId, completeTarget, boardClient, logger, ct, templateContext, crossReferenceResolver);
            }
