...
1. **Diff base shift.** Each `agent_run` row stores a `state_entry_canonical_sha` — the gate-check diff base, which is the **merge-base between the card branch and mainline**. The gate diffs `state_entry_canonical_sha → worktree HEAD`, so it sees the card's own committed work (and any merge-conflict resolutions) but not mainline's progress merged in along the way.
   - **First claim** for `(tenant, card, state)`: the SHA is read from the canonical worktree's HEAD after the optional merge step — equal to the merge-base when the branch was freshly cut from mainline.
   - **Re-runs** in the same state copy the value forward from the earliest matching run, so a re-run gate sees cumulative committed work across runs, not just this run's possibly-empty diff.
   - **Mainline merge into the branch (Approach D):** the orchestrator merges `origin/<default-branch>` into existing card branches before each non-discard run. Whenever it does, that run advances `state_entry_canonical_sha` to the merged mainline SHA (the new merge-base) and broadcasts it to every `agent_run` row for `(tenant, card, state)` (`IRunStore.UpdateStateEntryShaForCardStateAsync`). Without this, the stale pre-merge SHA would surface every merged-in mainline commit as an "unrequested change" and the gate rejects work that was actually in scope. The "up to date" case still reports `origin/<default-branch>`'s SHA so a re-run can advance a stale base without merging anything.
   - If the column is ever null on a run that needs it, the gate falls back to `runStartCanonicalSha` (this run's HEAD after the merge step). Default branch is read via `git symbolic-ref --short refs/remotes/origin/HEAD` or the `rerun.defaultBranch` override; aiboard hard-fails with an operator hint if neither is set (`git remote set-head origin --auto` to fix). This mechanism is provider-agnostic — it relies only on a `origin` remote with a discoverable default branch, not on GitHub.
