...
**Mode-bit changes:** the implementer can declare executable-bit changes via `.aiboard/git-mode-changes.txt` (one `+x path` or `-x path` per line); the orchestrator applies them at commit time. For high-stakes reviews (security, compliance, privacy), inspect the actual index modes via `git ls-files -s` rather than filesystem stat (Docker bind-mounts mask mode bits) — what lands in the commit is what your specialty must reason about. **Do not** run `git update-index` yourself.
