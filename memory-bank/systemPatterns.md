...
- **Mode-bit changes via `.aiboard/git-mode-changes.txt` manifest** (v0.0.26+): cross-platform agent runs (Windows host + Linux Docker) cannot reliably propagate executable bits through `git add` — NTFS doesn't store the bit, Windows git defaults to `core.fileMode=false`, and Docker bind-mount's bit-visibility on the host is unreliable. Agents declare mode changes by appending `+x path/from/repo/root` or `-x path/from/repo/root` to `.aiboard/git-mode-changes.txt` (one entry per line; blank lines and `#` comments allowed). `GitWorkspaceManager.CommitAsync` reads the manifest after `git add .` and before `git commit`, calling `git update-index --chmod=+x|-x <path>` per entry. `git update-index --chmod` is stat-independent — directly mutates the index entry's mode — so it works regardless of host OS. Per-entry failures (path not in index, malformed line, absolute or `..`-traversal path) log a warning and continue; never abort the surrounding commit. Path validation rejects `Path.IsPathRooted` and any path segment containing `..` upstream of git. The manifest itself lives under `.aiboard/` (gitignored) so it never reaches the commit. Wired through `CommitAsync` so single-agent commits AND per-candidate commits in `CandidateExecutor.ExecuteSingleCandidateAsync` get it for free. Discard-mode states (no commit) are a no-op. Motivating case: KvA issue #13 (CRLF line endings in shell scripts) — implementer chmod-ed inside Docker, orchestrator's host-side `git add` didn't see the bit, scripts landed at `100644` in the index, QA correctly flagged the gap on every re-run but no agent could fix it under the prior architecture (Git Policy forbade git writes; the only working command was `git update-index --chmod=+x` which agents wouldn't run). The manifest puts the declaration on the agent and the execution on the orchestrator. Pinned by `GitWorkspaceManagerModeManifestTests` (12 cases including the KvA #13 regression target, multiple entries, comments/blanks, malformed lines, missing-path-in-index warning, absolute-path/`..`-traversal rejection, idempotent re-application). System prompts (senior_engineer, code_reviewer, qa, gate_checker, estimator, specialist_reviewer, senior_specialist_reviewer) updated with a "Mode-bit changes" clause in their Git Policy sections; board_analyst.md is read-only and intentionally not updated.
...
- **Stdout NDJSON detection (post-KvA-2026-05-10 fix).** Claude CLI 2.1.x in `--output-format stream-json` mode emits the machine-readable rate-limit signal as a structured event in **stdout**, not stderr — shape `{"type":"rate_limit_event","rate_limit_info":{"status":"rejected","rateLimitType":"five_hour","overageStatus":"rejected","overageDisabledReason":"out_of_credits"}}`. The full 5-hour-window-rejected and out-of-credits failure shapes have empty stderr and only init + that event in stdout, so a stderr-only detector misses them. `AgentOutputParser.HasRejectedRateLimitEvent(stdout)` walks the NDJSON, returns true on any `rate_limit_event` whose `rate_limit_info.status` is anything other than `"allowed"` (forward-compatible with hypothetical `"queued"`/`"throttled"` states). Type-discriminated — agent prose mentioning "rate limit" never matches. Malformed lines are skipped, never thrown. `ClaudeAgentExecutor.IsRateLimitedStdout(stdout)` is a thin shim over the parser helper for symmetry with `IsRateLimited(stderr)` and reuse by the Docker executors. Wired into the exit-non-zero branch of all three Claude-family executors (`ClaudeAgentExecutor`, `DockerClaudeAgentExecutor`, `DockerClaudeQwenAgentExecutor`); Docker-exit-code classification (125/126/127/137) takes precedence over the stdout rate-limit check so a flaky daemon can't masquerade as a rate limit and loop the card-restoration path. Pre-fix this slipped through and got mis-classified as `AGENT_ERROR`, the card moved to **Problems** instead of being held, `agent_run.failure_reason` was NULL, and `rate_limit_events = 0` — the entire rate-limit recovery + telemetry path went silent. Codex's NDJSON shape doesn't include this event so it doesn't need the equivalent.
...
| AgentRunner | Direct agent execution: fetch cards → worktree → steps → post-process. Per-step branch into single-agent (existing path) vs slot-driven (delegates to CandidateExecutor.ExecuteSlotAsync per slot in order) when `step.GetEffectiveSlots()` returns non-empty. Slot loop: try slot 0; on `SlotOutcome.Failed` continue to next slot; on `Won` or `NeedsInfo` short-circuit and surface that slot's `AgentResult`. Last-slot's result is surfaced if all slots fail. `GetEffectiveSlots()` adapts legacy step-level `Candidates`+`Evaluator` to a single-element slot list, so the runtime sees only one shape regardless of which form the workflow JSON used. Prior-step context filters to canonical rows only: excludes rows with `candidate_group_id`/`candidate_index`, `:cand-` step names, and `:evaluator` rows from `.aiboard/context/step-history.md` and gate `{StepHistory}`. |
| CandidateExecutor | Optional service. `ExecuteSlotAsync(SlotConfig, slotIndex, totalSlots, request, ct) → SlotResult` runs ONE slot end-to-end: spawns N candidate branches/worktrees off canonical HEAD via `GitWorkspaceManager.CreateWorktreeFromStartPointAsync`, runs each candidate against its own worktree (**fully parallel — every candidate runs concurrently regardless of provider** via a single `Task.WhenAll` over the flat candidate list; wall-clock bounded by the slowest single candidate, not by per-provider group totals. Pre-v0.0.24 the executor grouped by provider and serialised same-provider candidates within each group on the rationale of "avoid per-provider rate-limit pile-ups"; in practice this cost ~17 minutes of wall-clock per design step on configs with several Claude models and accomplished little — Anthropic's account-level rate limit handles concurrent requests within tier, and 429s are already converted to per-candidate ERROR-with-rate-limit-flag by `ExecuteCandidateWithRetriesAsync` for slot/chain-level fallback. **Genuine** concurrency caps (a single shared local llama.cpp server backing both `docker-opencode` and `docker-claude-qwen`) are expressed declaratively via `IResourcePool` instead of reflexively via grouping. Original candidate-index is preserved in the returned `executions` list so downstream consumers — evaluator's "Candidate N", `step_result.candidate_index`, branch naming — see no order change regardless of completion order), persists per-candidate `step_result` rows (`candidate_group_id`/`candidate_index`/`provider`/`slot_index`), runs the evaluator (separate `step_result` with name suffix `:evaluator` plus `:slot-N` infix when multi-slot, no `candidate_group_id`), parses `winner_index` + `scores`, calls `IRunStore.UpdateCandidateEvaluationAsync` per row, promotes the winner. Evaluator failures before a valid verdict exists (CLI/auth/parse/timeout, `COMPLETE` without usable `winner_index`, etc.) walk the evaluator role fallback chain; a valid evaluator `outcome=ERROR` is a real no-winner judgment and does not fallback. **Per-candidate retry loop** in `ExecuteSingleCandidateAsync` wraps `executor.ExecuteAsync` and retries `RateLimitException`/`TimeoutException` up to `CandidateOverride.Retries` times when `CandidateOverride.RetryOn` includes the failure category (defaults to `[RATE_LIMIT, TIMEOUT]`). Bounded backoff: 30s base, 2× exponential, 5min cap, ±20% jitter. Failed retries are NOT persisted as separate `step_result` rows — only the eventual outcome is. **Single-candidate slot with `Evaluator = null`**: phase 1b short-circuit handles the candidate's outcome directly (COMPLETE → Won + promote, NEEDS_INFO → NeedsInfo + cleanup, ERROR → Failed + cleanup). Checked BEFORE the all-error multi-candidate short-circuit so a sole NEEDS_INFO surfaces as NeedsInfo rather than Failed. **Promotion mode forks on `gitBehavior`**: commit modes do `git reset --hard {winner-branch}` on canonical (winner's commits live on canonical's HEAD post-promotion); discard mode does **file-based promotion** — `PromoteDiscardWinnerArtifacts` clears canonical's `.aiboard/{tasks,updates}/` then copies the winner's contents in (so a winner that *removed* a file is reflected; canonical's git state unchanged). **All candidate worktrees AND branches are torn down after promotion in BOTH modes** via the unified `CleanupCandidateWorktreesAsync` (winner's branch included). Pre-v0.0.23 the commit-mode path preserved the winner's branch under a "post-flight git diagnostics" rationale that nothing actually used — every candidate run accumulated permanent `aiboard-cand/...` branches in the operator's repo (the v0.0.22 KvA field-report symptom). After promotion, AgentRunner's normal post-step processors (TaskFileManager, UpdateFileProcessor) read the promoted outputs from canonical the same way they would for a single-agent step. **Evaluator prompt material also forks on `gitBehavior`**: commit modes get `git diff` per candidate (diffed against `slotStartCanonicalSha` — the canonical HEAD SHA captured at slot start *before* candidates spawned, NOT against the candidate's own HEAD; `git diff HEAD` would be empty post-orchestrator-commit because each candidate is committed onto its own branch by `ExecuteSingleCandidateAsync` before the evaluator runs); discard modes get `.aiboard/tasks/{cardId}.md` content + `.aiboard/updates/*.md` listing (because `.aiboard/` is gitignored — diff is empty for discard candidates). Same diff-base-ref mechanism plumbs through to `AgentRunner.RunGateCheckAsync`: the gate diff base is `agent_run.state_entry_canonical_sha` (falls back to `runStartCanonicalSha` — canonical HEAD SHA captured after the optional merge step but before the step loop — only when that column is null) so the gate check after a candidate-group promotion still surfaces the run's cumulative work despite `git reset --hard` resetting canonical's working tree to match HEAD. `state_entry_canonical_sha` is the merge-base between the card branch and mainline: carried forward across re-runs AND **advanced to the merged mainline SHA whenever the pre-step merge step pulls `origin/<default-branch>` into the branch** (Approach D — `GitWorkspaceManager.MergeMainBranchAsync` reports the merged SHA on `MergeMainResult.MergedMainSha`, `AgentRunner` overrides + persists it via `IRunStore.SetStateEntryShaAsync` and broadcasts to every `(card,state)` run via `IRunStore.UpdateStateEntryShaForCardStateAsync`), so the gate diff never surfaces merged-in mainline progress as "unrequested changes". Provider-agnostic — relies only on a `origin` remote with a discoverable default branch (`git symbolic-ref refs/remotes/origin/HEAD` or `rerun.defaultBranch`), not GitHub. Multi-candidate short-circuit only when every candidate returned `ERROR`; `NEEDS_INFO` is evaluator-eligible and ranked with `COMPLETE`. If the evaluator selects a NEEDS_INFO winner, the slot returns NeedsInfo and does not fall back. Phases 3–5 (verdict parse, persist, promote, comments) are wrapped in `try/finally` so the evaluator's inline-system-prompt temp file (`$TMP/aiboard-evaluator-{guid}.md`) is always deleted even when later phases throw. Candidate branch shape: `aiboard-cand/{cardId}-{groupShort}-{index}-{provider}` (deliberately a sibling-of-canonical, NOT nested under the canonical worktree path); built once via the static helper `BuildCandidateBranchName` and pre-computed for every candidate at slot start so the slot-throw cleanup catch can sweep ALL predicted branches (including in-flight / faulted candidate tasks whose `CandidateExecution` never returned). `CleanupCandidateWorktreesAsync` and `GitWorkspaceManager.RemoveWorktreeAsync` are best-effort throughout: the latter never throws on non-cancellation failures, runs `git worktree prune` unconditionally to clear stale registrations from partial cleanups, and treats branch deletion as independent of worktree removal so a locked worktree directory doesn't prevent the branch ref from being cleared. |
| RerunCacheGate | Deterministic-skip cache decision (rerun redesign Problem 1). `EvaluateAsync` builds an input-bundle hash (operator description prefix, operator non-aiboard-log comments, prior section hashes, step config JSON, prompts) and a managed-section hash, looks up the most recent `COMPLETE` `step_result` for `(card, state, step)` with `candidate_group_id IS NULL`, returns `IsHit = true` when both hashes match. AgentRunner skips the LLM call on hit and emits a `kind:cache_hit` comment. Replaces the deleted `RerunPreambleBuilder` (LLM-judgment fast-path). Comment-based force-rerun is not a thing — only operator-authored inputs participate in the hash. |
...
| DockerMountBuilderBase / DockerClaudeMountBuilder / DockerCodexMountBuilder / DockerOpenCodeMountBuilder / DockerClaudeQwenMountBuilder | Builds workspace/`.git` bind mounts (base, shared) and provider-specific extras. Claude builder copies the host's real `~/.claude` into a per-run staging dir mounted RW at `/home/agent/.claude`. Codex builder copies allowlisted top-level credential files from `~/.codex` to a per-run staged dir and mounts each individually as **read-only** under `/home/agent/.codex/{filename}` (the directory itself is image-baked agent-owned in `docker/codex-sandbox/Dockerfile`). The allowlist (`CredentialFileNames`: `auth.json`, `config.toml`, `cap_sid`, `version.json`, `.personality_migration`) was originally added in v0.0.23 with `installation_id` included; Phase 2 removed `installation_id` after Codex 0.128.0 started failing EROFS on the RO mount (the file is now provided via `CODEX_INSTALLATION_ID` env var and written by the sandbox entrypoint). Pre-v0.0.23 every top-level file was mounted, which broke Codex CLI 0.125.0+ because it needed to write to `models_cache.json` / `state_*.sqlite*` / `logs_*.sqlite*` / `sandbox.log` / `history.jsonl` at startup and the RO mounts blocked it (KvA card 4 polling-run failure: DOA in <3s with `Read-only file system (os error 30)`). Runtime state files are now neither copied nor mounted — Codex regenerates fresh copies in the agent-writable container directory on every run. Excludes heavy subdirs `sessions`/`log`/`screenshots` from the recursive copy. Host directory never mutated; subdirs that survive the copy are intentionally NOT mounted (re-introducing a dir-level mount would re-trigger the v0.0.22 KvA Windows + Docker Desktop perm bug). OpenCode builder adds no extra mounts (connection passed as env vars only). Claude→Qwen builder writes a **synthetic** `~/.claude` (settings.json with `hasCompletedOnboarding=true` + dummy credentials.json) to a temp dir and mounts it RW — must NOT mount the operator's real `~/.claude` because that would leak Anthropic credentials into a Qwen-target run. All inject env vars through `DockerMountContext.EnvironmentVariables`. |
...
| fast_path_hit | BOOLEAN NULL — **deprecated** (Round-4 rerun redesign). Was populated by the removed `RerunPreambleBuilder`; column preserved on the schema for back-compat but no longer written. Cache decision now flows through `execution_kind` ('cache_hit' vs 'full_run'). V22 |
| execution_kind | TEXT NOT NULL DEFAULT 'full_run' — 'cache_hit' when the deterministic cache reused a prior step_result; 'full_run' otherwise. Read by `v_cache_hit_rate`. V24 |
| output_summary | TEXT NULL — short narrative captured at step completion; reused as the synthetic detail when a future run cache-hits this row. V24 |
| input_hash | TEXT NULL — SHA-256 of the operator-side input bundle (description prefix, non-aiboard-log comments, prior section hashes, step config, prompts). Used by `RerunCacheGate` to detect "nothing relevant changed." V24 |
| section_output_hash | TEXT NULL — SHA-256 of the step's managed-section content after the agent ran. Used by the cache to detect operator edits to a step's own section between runs. V24 |
| section_update_json | JSONB NULL — raw `section_update` directive returned by the agent (replay / debugging). V24 |
| source_run_id / source_step_result_id | TEXT NULL / UUID NULL — back-pointers from cache_hit rows to the source row whose result was reused. V24 |
...
| `v_cache_hit_rate` | Per-`(state, step, role, provider)` deterministic-cache hit rate (V25). Ratio of rows where `execution_kind = 'cache_hit'` over all `full_run`/`cache_hit` rows. High rate = cache is paying off; low rate on a step that re-runs often signals input-bundle drift (operator-edited managed sections, comment churn, prompt or model changes). Replaced `v_fast_path_hit_rate` (dropped in V25 with the removed preamble). |
