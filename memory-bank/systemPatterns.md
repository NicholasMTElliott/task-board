...
- `DependencyGuard`: runs before IN_PROGRESS transition. Polling skips blocked cards and selects the next eligible card. Direct agent and merge runs return `NEEDS_INFO` without moving the card. Per-cycle hydration cache (`Dictionary<string, BoardCard>?` parameter on `CheckAsync`) — `PollingRunner` allocates one per cycle so two enforced cards sharing a blocker only fetch the blocker's card body once. Closed-and-completed blockers short-circuit hydration entirely (column lookup unnecessary). Blocked-comment marker: `<!-- agent-dependency-blocked -->`. Read-only callers (`DiagnoseRunner`) pass `recordSideEffects: false` to the full-control `CheckAsync` overload — the result is returned without writing to `card_dependency_wait`, posting the blocked-comment, or logging the blocking decision.
