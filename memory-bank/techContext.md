...
| Agent executor (local LLM) | OpenCode CLI inside Docker | Via `DockerOpenCodeAgentExecutor`; provider key `docker-opencode`; image `aiboard-opencode-sandbox:latest`; targets a local llama.cpp server on the `llm-net` bridge network (e.g. Qwen3.6 via the sibling `local-llm` project) |
