...
public class MergeRunnerTests : IDisposable
{
    ...
    public async Task ExecuteAsync_WrongGateType_ReturnsError()
    {
        ...
        var runner = new MergeRunner(_boardClient, _gitManager, config, new AgentIdentity("Agent", "TestMachine"), Substitute.For<ICrossReferenceResolver>(), Substitute.For<IAgentExecutorResolver>(), NullLogger<MergeRunner>.Instance);
        var result = await runner.ExecuteAsync(CardId, BoardId, "C:/fake/path", CancellationToken.None);
    ...
    private MergeRunner CreateRunner(IAgentExecutorResolver? executorResolver = null) =>
        new(_boardClient, _gitManager, _config, new AgentIdentity("Agent", "TestMachine"),
