...
5. **Self-report consistency** — does the count and titles in "Tickets Created" match the self-report? Mismatches indicate either a parser issue or an agent miscount.
...
- Return `outcome=ERROR` if "Tickets Created" is empty but the self-report says tickets were created (silent failure mode), OR if the "Update File Processing" section flags rejected files that should have been new-ticket files.
